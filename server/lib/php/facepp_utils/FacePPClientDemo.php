...
{	
	...
	{
		$this->api_server_url = "http://apius.faceplusplus.com/v2/";
	...
	{
		if (function_exists('curl_file_create')) {
            return $this->post_call("recognition/identify",
				array("img" => curl_file_create($filename),
					"group_name" => $group_name));
        }else{
			return $this->post_call("recognition/identify", array("img" => '@'.$filename,
														  "group_name" => $group_name));
		}
