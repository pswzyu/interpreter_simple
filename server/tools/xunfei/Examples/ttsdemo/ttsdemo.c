	...
	while (1) 
	{
		...
		if (NULL != data)
		{
		   ...
		   pcmwavhdr.data_size += audio_len;//修正pcm数据的大小
              ...
              //printf("\nget audio...\n");
              usleep(1000);//建议可以sleep下，因为只有云端有音频合成数据，audioget都能获取到音频。
...
{
	///APPID请勿随意改动
	const char* login_configs = " appid = 5435cbf7, work_dir =   .  ";
	...
	// concatenate all argvs after second parameter
	unsigned int i;
	size_t len = 0;
	...
	*(_all_args-1) = 0;
	...
	//音频合成
	ret = text_to_speech(all_args,argv[1],param);
	if ( ret != MSP_SUCCESS )
	{
